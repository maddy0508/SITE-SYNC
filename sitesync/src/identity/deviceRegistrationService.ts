...
import { getLocalDeviceSession, insertLocalDeviceSession, updateLocalDeviceSession } from '../database/localPersistence';

export type DeviceRegistrationErrorCode = 'INVALID_INPUT' | 'DEVICE_NOT_FOUND' | 'CROSS_USER_ACCESS' | 'SUPABASE_FAILED' | 'INVALID_LIFECYCLE' | 'REVISION_MISMATCH' | 'LOCAL_DEVICE_CONFLICT';
...
type LocalPersistenceBoundary = Pick<typeof import('../database/localPersistence'), 'getLocalDeviceSession' | 'insertLocalDeviceSession' | 'updateLocalDeviceSession'>;

export class DeviceRegistrationService {
  constructor(private readonly client: SupabaseClient, private readonly local: LocalPersistenceBoundary = { getLocalDeviceSession, insertLocalDeviceSession, updateLocalDeviceSession }) {}
  ...
  async register(authenticatedUserId: string, input: DeviceRegistrationInput): Promise<DeviceInstallation> {
    ...
    if (localSession && localSession.installationKey !== input.installationKey) throw new DeviceRegistrationServiceError('LOCAL_DEVICE_CONFLICT', 'A different device installation is already authoritative in local persistence');

    const existing = await this.client.from('device_installations').select('id, user_id, installation_key, device_name, app_version, os_version, status, created_at, last_seen_at, revoked_at').eq('user_id', authenticatedUserId).eq('installation_key', input.installationKey).maybeSingle() as unknown as QueryResult<Row>;
    if (existing.error) throw new DeviceRegistrationServiceError('SUPABASE_FAILED', existing.error.message);
    if (existing.data) {
      const installation = this.mapInstallation(existing.data, authenticatedUserId);
      if (installation.status === 'ACTIVE') return this.syncLocalSession(installation, input.now);
      return installation;
    }
    if (localSession) throw new DeviceRegistrationServiceError('DEVICE_NOT_FOUND', 'Local device session exists but its authoritative installation could not be resolved');

    const inserted = await this.client.from('device_installations').insert({ user_id: authenticatedUserId, installation_key: input.installationKey, device_name: input.deviceName ?? null, app_version: input.appVersion ?? null, os_version: input.osVersion ?? null, status: 'ACTIVE', last_seen_at: input.now }).select('id, user_id, installation_key, device_name, app_version, os_version, status, created_at, last_seen_at, revoked_at').single() as unknown as QueryResult<Row>;
    if (inserted.error) throw new DeviceRegistrationServiceError('SUPABASE_FAILED', inserted.error.message);
    if (!inserted.data) throw new DeviceRegistrationServiceError('SUPABASE_FAILED', 'Device registration returned no installation');

    const installation = this.mapInstallation(inserted.data, authenticatedUserId);
    await this.local.insertLocalDeviceSession({ userId: authenticatedUserId, deviceInstallationId: installation.id, installationKey: installation.installationKey, status: installation.status, createdAt: installation.createdAt, lastVerifiedAt: input.now, updatedAt: input.now });
    return installation;
  }
  ...
  private async syncLocalSession(installation: DeviceInstallation, now: string): Promise<DeviceInstallation> {
    ...
    if (!localSession) {
      await this.local.insertLocalDeviceSession({ userId: installation.userId, deviceInstallationId: installation.id, installationKey: installation.installationKey, status: installation.status, createdAt: installation.createdAt, lastVerifiedAt: now, updatedAt: now });
      return installation;
    }
    if (localSession.deviceInstallationId !== installation.id || localSession.installationKey !== installation.installationKey) throw new DeviceRegistrationServiceError('CROSS_USER_ACCESS', 'Local device session does not match authoritative installation');
    await this.local.updateLocalDeviceSession(localSession.userId, localSession.revision, { lastVerifiedAt: now, updatedAt: now });
    return installation;
  }
  ...
  private normalizeTimestamp(value: unknown, field: string): string {
    ...
    if (Number.isNaN(parsed.getTime())) throw new DeviceRegistrationServiceError('SUPABASE_FAILED', `Invalid ${field} timestamp returned by Supabase`);
    return parsed.toISOString();
  }

  private validateInput(authenticatedUserId: string, input: DeviceRegistrationInput): void {
    if (!authenticatedUserId) throw new DeviceRegistrationServiceError('INVALID_INPUT', 'Authenticated user id is required');
    if (!input.installationKey || !input.now) throw new DeviceRegistrationServiceError('INVALID_INPUT', 'Installation key and timestamp are required');
    const parsed = new Date(input.now);
    if (Number.isNaN(parsed.getTime()) || parsed.toISOString() !== input.now) throw new DeviceRegistrationServiceError('INVALID_INPUT', 'Timestamp must be a normalized UTC ISO-8601 value');
  }
}
