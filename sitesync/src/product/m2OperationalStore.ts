...
function serialise(value: M2OperationalProject): string { return JSON.stringify(value); }
  ...
  async save(value) {
    ...
    await getDb().execute(
      `INSERT INTO m2_operational_project_projection (project_id, organisation_id, company_id, payload_json, updated_at)
       VALUES (?, ?, ?, ?, ?) ON CONFLICT(project_id) DO UPDATE SET organisation_id=excluded.organisation_id,
       company_id=excluded.company_id, payload_json=excluded.payload_json, updated_at=excluded.updated_at`,
  ...
  async load(projectId, organisationId, companyId) {
    ...
    const result = await getDb().execute(
      `SELECT organisation_id as organisationId, company_id as companyId, payload_json as payloadJson
       FROM m2_operational_project_projection WHERE project_id = ?`, [projectId]);
