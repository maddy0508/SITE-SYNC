-- Follow-up hardening migration for PL/pgSQL parameter/column ambiguity.
...
BEGIN
    ...
    IF actor_org IS NULL OR NOT public.sitesync_sync_authorized(
        sync_attendance_command.device_installation_id,
        sync_attendance_command.project_id,
        sync_attendance_command.person_id
    ) THEN
    ...
    WHERE pa.id = (payload->>'projectAssignmentId')::UUID
      AND pa.project_id = sync_attendance_command.project_id
      AND pa.person_id = sync_attendance_command.person_id
    ...
    INSERT INTO public.sitesync_attendance_day (
        ...
        sync_attendance_command.project_id, sync_attendance_command.person_id, sync_attendance_command.work_date_utc,
        actor_org, target_assignment.company_id, target_assignment.id, 'CHECKED_OUT', '{}'::JSONB, 0
