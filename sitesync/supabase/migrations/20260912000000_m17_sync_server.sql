...
CREATE INDEX sitesync_attendance_day_aggregate_idx ON public.sitesync_attendance_day (project_id, person_id, work_date_utc);
CREATE INDEX sitesync_sync_command_receipt_aggregate_idx ON public.sitesync_sync_command_receipt (project_id, person_id, work_date_utc);
CREATE INDEX sitesync_attendance_event_aggregate_idx ON public.sitesync_attendance_event (project_id, person_id, work_date_utc, new_revision);
...
-- No direct client table access: RLS is enabled and all table privileges are revoked.
...
BEGIN
    ...
    EXCEPTION WHEN others THEN
        RETURN jsonb_build_object('status','VALIDATION_REJECTED','code','INVALID_PAYLOAD','message','Payload identity or timestamp is invalid');
    END;
    ...
    WHERE pa.id = (payload->>'projectAssignmentId')::UUID
      AND pa.project_id = project_id AND pa.person_id = person_id
      AND pa.organisation_id = actor_org AND pa.status = 'ACTIVE';
    IF NOT FOUND THEN
    ...
    -- Re-check idempotency after the aggregate lock so concurrent duplicate delivery cannot race the receipt insert.
    SELECT * INTO receipt
    FROM public.sitesync_sync_command_receipt r
    WHERE r.command_id = sync_attendance_command.command_id;
    IF FOUND THEN
    ...
    aggregate.authoritative_payload := jsonb_build_object(
        ...
        'source',payload->>'source','clientOccurredAt',occurred_at,'state',aggregate.state,
